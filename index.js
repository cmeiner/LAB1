...
app.get("/boxes/:ID", (req, res) => {
  ...
  if (!foundBox)
    return res.status(404).send("The requested box could not be found!");
...
app.put("/boxes/:ID", (req, res) => {
  ...
  fs.writeFile(
    boxFile,
    JSON.stringify(updatedBoxes, null, 2),
    function writeJSON(err) {
      if (err) return console.error(err);
  ...
  return res.send(
    "Uppdaterade box#" + boxID + " med ändringen: " + JSON.stringify(updatedBox)
  );
});
